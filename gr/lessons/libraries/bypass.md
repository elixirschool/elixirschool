...
Σε αυτό το μάθημα θα εξερενήσουμε πως η [bypass](https://github.com/PSPDFKit-labs/bypass) μπορεί να μας βοηθήσει να χηριστούμε γρήγορα και εύκολα αυτά τα αιτήματα στις δοκιμές μας.
...
Για να το κάνουμε αυτό θα δημιουργήσουμε ένα πρότζεκτ επίβλεψης και έναν GenServer για να ελέγξουμε τους τομείς σε ένα διαμορφοποιούμενο διάστημα.
Χρησιμοποιόντας την Bypass στις δοκιμές μας θα έχουμε την δυνατότητα να επιβεβαιώσουμε πως η εφαρμογή μας θα λειτουργεί με πολλά διαφορετικά αποτελέσματα.
...
Σε υτό το σημείο θα πρέπει να έχουμε άνεση με το να δημιουργούμε νέα Mix πρότζεκτ και να προσθέτουμε τις εξαρτήσεις μας, οπότε αντ' αυτού θα εστιάσουμε στα κομμάτια του κώδικα που θα δοκιμάζουμε.
...
Ας αρχίσουμε φημιουργόντας μια νέα ενότητα που θα χειρίζετε τα εξερχόμενα αιτήματά μας προς τους τομείς.
Ας δημιουργήσουμε μια συνάρτηση με τη βοήθεια του [HTTPoison](https://github.com/edgurgel/httpoison), την `ping/1`, που θα δέχεται ένα URL και θα επιστρέφει `{:ok, body}` για αιτήματα HTTP 200 και `{:error, reason}` για τα υπόλοιπα:
...
Θα παρατηρείσετε οτι _δεν_ φτιάχνουμε έναν GenServer και υπάρχει ένας καλός λόγος γι' αυτό:
...
Γαι να προετοιμάσουμε την Bypass ώστε να δέχεται αιτήματα, πρέπει να ανοίξουμε την σύνδεση με το `Bypass.open/1`, το οποίο μπορεί να γίνει στον ορισμό επανάκλησης της δοκιμής μας:
...
Μπορούμε να αρχήσουμε με μια δοκιμή σχετικά ίδια με την πρώτη με πολύ μικρές αλλαγές: να επιστρέφει 500 στην κατάσταση του κώδικα και στην βεβαίωση ισότητας να επιστρέφει την τούπλα `{:error, reason}`:
...
Δεν υπάρχει κάτι ιδιαίτερο σε αυτή την περίπτωση δοκιμής οπότε ας περάσουμε στο επόμενο: απροσδόκητες διακοπές λειτουργίας του εξυπηρετητή, αυτά είναι τα αιτήματα που μας ενδιαφέρουν περισσότερο.
Για να το επιτύχουμε αυτό δεν θα χρησιμοποιήσουμε την `Bypass.expect/2` αντ' αυτής θα βασιστούμε στην `Bypass.down/1` ώστε να κλείσει η σύνδεση:
...
Έχοντας δοκιμάσει την ενότητά μας `HealthCheck` μπορούμε να προχωρίσουμε σητν δοκιμή της παράλληλα με τον βασισμένο σε GenServer προγραμματιστή μας.
...
Περνόντας στην ενότητά μας αποσυνδέουμε περαιτέρω την λειτουργικότητα μας και τον GenServer, δίνοντάς μας την δυνατότητα να δοκιμάσουμε ξεχωριστά το κάθε τμήμα:
...
Όπως συζητήσαμε περνάμε τις σελίδες μας στο `HealthCheck.ping/1` και μετά θα επαναλάβουμε τα αποτελέσματά μας με την `Enum.each/2` εφαρμόζοντας σε κάθε ένα την συνάρτησή μας `report/1`.
Με αυτές τις συναρτήσεις έτοιμες ο προγραμματιστής μας είναι έτοιμοσ και μπορούμε πλέον να εστιάσουμε στην δοκιμή του.

Δεν θα εστιάσουμε τόσο στην μοναδιαία δοκιμή του προγραμματιστή, καθώς αυτό δεν απαιτεί την χρήση της Bypass, οπότε μπορούμε να περάσουμε στον τελικό κώδικα:
...
Βασιζόμαστε σε μια εφαρμογή δοκιμών με την χρήση του `TestCheck` παράλληλα με το `CaptureLog.capture_log/1` ώστε να βεβαιώσει ότι τα σωστά μηνύματα καταγράφοντε.
...
Θα χρειαστούμε την Bypass για αυτή την δοκιμή και θα πρέπει να χειριστούμε πολλαπλά αιτήματα Bypass ανα δοκιμή, ας δούμε πως θα το κάνουμε αυτό.
