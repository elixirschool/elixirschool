...
* Η συνάρτηση που θα εκτελεστεί από την απομακρυσμένη διεργασία που τρέχει σε αυτό τον κόμβο
...
Τώρα, μπορούμε να χρησιμοποιήσουμε την [`Node.spawn_link/2`](https://hexdocs.pm/elixir/Node.html#spawn_link/2) για να έχετε τον κόμβο `alex` να ρωτήσει τον κόμβο `kate` να καλέσει τη συνάρτηση `say_name/0`:
...
Αυτό μας επιτρέπει να εκτελούμε εργαίες εισόδου/εξόδου, σαν την `IO.puts`, μεταξύ κατανεμημένων κόμβων.
...
Τι συμβαίνει αν θέλουμε ο κόμβος που δέχεται το μήνυμα να στείλει πίσω κάποι *απάντηση* στον αποστολές; Μπορούμε να χρησιμοποιήσουμε μια απλή διάταξη `receive/1` και [`send/3`](https://hexdocs.pm/elixir/Process.html#send/3) για να καταφέρουμε ακριβώς αυτό.
...
Ας κάνουμε την εφαρμογή μας Chat λιγό περισσότερο έξυπνη.
...
Έτσι, θα μάθουμε την ενότητά μας `Chat` πως να απαντά σε κάθε μήνυμα που στέλνεται σε έναν κόμβο με όνομα `moebi@localhost` για λογαριασμού του Moebi.
...
Πολύ δουλειά και σίγουρα δεν μπορεί να θεωρηηθεί μια αυτοματοποιημένη διαδικασία δοκιμής.

Υπάρχουν δύο διαφορετικές προσεγγίσεις που μπορούμε να πάρουμε εδώ:

1. Προεραιτικά να αποκλείσουμε δοκιμές που απαιτούν κατανεμημένους κόμβους, αν ο απαιτούμενος κόμβος δεν τρέχει.
...
Η `Task.Supervisor.async/5` δέχεται ένα πρώτο όρισμα του επητηρητή που θέλουμε να χρησιμοποιήσουμε.
Αν περάσουμε μια τούπλα `{SupervisorName, location}`, θα εκκινήσει τον δεδομένο επιτηρητή στο δεδομένο απομακρυσμένο κόμβο.
Παρόλα αυτά, αν περάσουμε στον `Task.Supervisor` σαν πρώτο όρισμα το όνομα ενός επιτηρητή, θα χρησιμοποιήσει αυτό τον επιτηρητή για να επιτηρήσει την εργασία τοπικά.
...
Μπορούμε να φανταστούμε μπορούμε να χρησιμοποιήσουμε την ικανότητα της Elixir για κατανεμημένη επεξεργασία για να τρέχουμε ταυτόχρονες εργασίες παρασκηνίου, για να υποστηρίζουμε εφαρμογες υψηλής απόδοσης, για να τρέχουμε ακριβές σε επεξεργασία λειτουργίες - ότι θέλουμε.

Αυτό το μάθημα μας δίνει μια βασική εισαγωγή στην έννοια της κατανομή στην Elixir και μας δίνει τα εργαλεία που χρειαζόμαστε για να ξεκινήσουμε να χτίζουμε κατανεμημένες εφαρμογές.
