...
Встановлення мови программування, базові типи даних та прості операції.
...
Для того щоб почати, давайте запустимо утиліту `iex`:
...
Не турбуйтеся, якщо ви поки що не розумієте якийсь вираз. Ми сподіваємося на те, що буде зрозуміло хоча б в цілому.
...
Elixir підтримує `true` та `false` як логічний тип. В мові усе істинне окрім `false` та `nil`:
...
Атом - це константа, ім'ям якої є її значення. Якщо ви знайомі з мовою программування Ruby, то це схоже на символи:
...
Рядки в Elixir кодуються за допомогою UTF-8 та об’являються за допомогою подвійних лапок:
...
Elixir має такі оператори: `||`, `&&`, `!` . Вони підтримують будь-який тип аргументів:
...
Якщо ви користувалися мовою Ruby, інтерполяція в Elixir буде знайомою:
