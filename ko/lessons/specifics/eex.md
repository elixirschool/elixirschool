...
결과물을 출력하고 싶은 모든 표현 식은 __반드사__ 등호(`=`)를 사용해야 합니다. 다른 템플릿 언어에서는 `if`와 같은 철을 특별하게 처리하고 있지만, EEx는 그렇지 않기 때문에 중요합니다. `=`가 없다면 아무것도 출력되지 않을 것입니다:
...
자신만의 엔진을 만드는 것에 흥미가 있으신가요? 무엇이 필요한지 알아보기 위해, [`EEx.Engine`](http://elixir-lang.org/docs/v1.2/eex/EEx.Engine.html)가 어떻게 행동하는지 확인해보세요.
