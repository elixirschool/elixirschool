...
ETS는 Elixir나 Erlang에서 객체를 메모리에 담아둘 수 있는 견고한 저장소입니다. ETS는 무척 큰 데이터를 저장할 수 있으며, 상수 시간 접근을 제공합니다.
...
단순한 매치를 사용하는 경우에 대해서 배웠습니다. 좀 더 SQL 질의와 비슷한 건 없을까요? 다행스럽게도 더 강력한 구문을 사용할 수도 있습니다. 데이터를 `select/2`로 검색하려면, 3개의 인수를 가지는 튜플 리스트를 만들어야 합니다. 이 튜플은 패턴, 0이나 다수의 가드, 그리고 반환 형식을 표현합니다.
