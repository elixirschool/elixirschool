...
Erlang Term Storage, 줄여서 ETS는 OTP 내부에 포함된 강력한 저장용 엔진으로, Elixir에서 이용할 수 있습니다. 이 레슨에서는 ETS를 호출하는 방법과 애플리케이션에서 사용하는 방법에 관해서 설명합니다.

{% include toc.html %}

## 개요

ETS는 견고한 인-메모리 저장소로, Elixir나 Erlang의 객체를 저장하기 위한 것입니다. ETS는 무척 큰 데이터를 저장할 수 있으며, 상수 시간 접근을 제공합니다.
...
단순한 매치를 사용하는 경우에 대해서 배웠습니다. 좀 더 SQL 질의와 비슷한 건 없을까요? 감사하게도 더욱 견실하게 되어 있는 구문을 사용할 수 있습니다. 데이터를 `select/2`로 검색하려면, 3개의 인수를 가지는 튜플 리스트를 만들어야 합니다. 이 튜플은 패턴, 0이나 다수의 가드, 그리고 반환 형식을 표현합니다.
...
레코드의 제거는 `insert/2`나 `lookup/2`와 마찬가지로 직접적입니다. `delete/2`로 테이블과 키를 지정하기만 하면 됩니다. 이 함수는 지정된 키와 값을 모두 삭제합니다:
...
지금까지 배운 내용을 활용해서, 비싼 처리를 위한 간단한 캐시를 만들어 봅시다. 모듈과 함수, 인수, 옵션을 받는 `get/4` 함수를 구현합니다. 옵션은 아직 `:ttl`만 신경 쓰면 됩니다.
