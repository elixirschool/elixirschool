...
흔히들 `Enum` 모듈을 이용할 때 `all?/2`을 사용하면, 컬렉션의 아이템에 적용할 함수를 넘기게 됩니다.
...
가끔 컬랙션을 뽑아내는 것으로 정확히 원하는 것을 얻기 힘들 때가 있습니다.
...
`filter/2` 함수를 이용하면, 주어진 함수를 사용하여 'true'로 평가되는 요소 만 포함하도록 컬렉션을 필터링합니다.
...
편의를 위해, `sort/2`는 정렬 함수 대신 `:asc`, `:desc`를 넘기는 것이 허용됩니다.
...
`uniq_by/2`도 열거가능한 집합에서 중복을 제거합니다만, 고유인지 비교할 함수을 넘길 수 있습니다.
...
Elixir의 Enum 모듈의 많은 함수는 전달되는 열거가능한 집합의 각 요소와 함께 작용하는 익명 함수를 인수로 사용합니다.

이 익명 함수는 자주 캡쳐 연산자(&)를 사용한 단축으로 작성됩니다.

캡쳐 연산자가 어떻게 Enum 모듈과 함께 구현에 사용되는지 예제를 통해 알아봅시다.
각 버전의 기능은 동일합니다.
...
좀 더 리팩토링을 하자면, 캡쳐 연산자를 미리 익명 함수를 변수에 할당하고 `Enum.map/2` 함수에서 호출하게 할 수 있습니다.
...
가장 간결한 구문은 명시적으로 변수를 캡처하지 않고 이름이 있는 함수를 직접 호출하는 것 입니다.
