...
이를 충족하기 위해, `@spec`과 비슷한 문법을 가진 `@callback` 디렉티브를 사용하겠습니다. 이 디렉티브로 __반드시 구현__되어야 하는 함수를 정의합니다. 매크로는 `@macrocallback`를 사용해서 할 수 있습니다. 워커를 위해 `init/1`, `perform/2` 함수를 정의해 봅니다.
