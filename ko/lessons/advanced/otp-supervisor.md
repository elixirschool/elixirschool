...
슈퍼바이저(Supervisor)는 다른 프로세스를 감시한다는, 단 하나의 목적에 특화된 프로세스입니다. 자식 프로세스가 실패하면 자동으로 새시작해주는 것으로, 장애에 대한 내성이 높은(Fault-tolerant) 애플리케이션을 만들 수 있습니다.
...
프로세스가 정지하거나, 종료되면 슈퍼바이저는 아무 일도 없었던 것처럼 자동적으로 프로세스를 재기동할 것입니다.
...
+ `:simple_one_for_one` - 동적으로 추가된 자식 프로세스에게 최적입니다. 슈퍼바이저는 단 하나만 자식 프로세스에 추가해야합니다.

### 중첩

워커 프로세스에 이어서, 슈퍼바이저에게 슈퍼바이저의 트리 구조를 생성하도록 시킬 수도 있습니다. 다른 점은 `worker/3`을 `supervisor/3`으로 변경하기만 하면 됩니다:
...
Task는 전용 슈퍼바이저인 `Task.Supervisor`를 가지고 있습니다. 동적으로 태스크를 생성하도록 설계되어 있으며, 내부적으로 `:simple_one_for_one`를 사용합니다.
