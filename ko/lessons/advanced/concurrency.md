...
새로운 프로세스를 생성하는 가장 쉬운 방법은 `spawn`입니다. `spawn`은 익명함수나 이름이 있는 함수를 인자로 받습니다. 프로세스를 새로 생성할 때 해당 프로세스가 어플리케이션 내에서 유일한지 식별하기 위해 _프로세스 식별자_, 다시 말해, PID를 반환합니다.

시작하기 전에, 모듈을 생성하고 그 내부에 동작시키고 싶은 함수를 정의합시다:
...
`spawn`을 다룰 때, 프로세스가 언제 충돌나는지도 생각해봐야 합니다. 이런 경우에는 `spawn_link`로 프로세스를 연결할 필요가 있습니다. 두 프로세스는 연결된 다른 프로세스로부터 종료 신호를 수신하게 될 것입니다.
...
연결된 프로세스가 다른 프로세스와 충돌하지 않고 싶을 때도 있습니다. 이런 경우에는, 종료 신호를 trap 해야할 필요가 있습니다. 종료 신호를 trap할 때, 다음과 같은 튜플 메시지를 수신하게 될 것입니다: `{:EXIT, from_pid, reason}`
...
두 프로세스를 서로 연결시키고 싶지 않지만, 이에 대한 정보를 계속해서 알림받고자 한다면 어떻게 하면 될까요? 이런 경우에는 `spawn_monitor`로 프로세스를 모니터링할 수 있습니다. 프로세스를 모니터링하면, 구동중인 프로세스를 충돌시키거나 종료신호를 명시적으로 trap할 필요없이 프로세스가 충돌여부를 알려주는 메시지를 얻게 됩니다.
...
에이전트(Agent)는 상태를 유지하는 백그라운드 프로세스를 추상화시킨 것입니다. 어플리케이션 내부의 다른 프로세스나 노드로 접근이 가능합니다. 다음의 예시에서, 에이전트의 상태는 함수의 반환 값으로 설정되어 있습니다:
