...
多くの認証システムが、リソースへの subject identifier だけへのアクセスを提供し、JWTはこれらの情報を提供します。
...
トークンがエンコードされていることを確認したら（それが正しく署名されていることを確認してください）、トークンの発行は完了です。

ただし、JWTを追跡するためにデータベースを使用することは __可能__ です。
...
ただし、ステージングと本番には、機密保持をより強力にする必要があります。
(例
...
最初のpipelineは、セッション内で最初にトークンを見つけ、次にヘッダーにフォールバックします。
...
ブラウザセッションのログインはhoodの下で `encode_and_sign` を呼び出すので、同じように使うことができます。