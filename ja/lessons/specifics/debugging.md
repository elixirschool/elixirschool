...
人が読むことができるドキュメント(それが存在し、良く書かれていれば)とは違い、`@spec`はツールによって理解される正式な構文を使用します。
...
このDialyzerのメッセージの内容は明らかです。`sum_times/1`関数の戻り値の型が定義されたものと異なっています。
...
ツールで仕様を使用するために静的コード解析を実行すると、自己テストされたバグの少ないコードを作成できます。
