...
`iex` セッションを起動して名前を与えることで、ノードを開始できます:
...
これら2つのノードは `Node.spawn_link/1` を使うことでもう一方にメッセージを送ることができます。

### `Node.spawn_link/1` による通信
...
受信したノードから送信側に対して何らかの *返信* を返したい場合はどうしましょう？ これは、単に `receive/1` と [`send/2`](https://hexdocs.pm/elixir/Process.html#send/3) を使うことで実現できます。
...
これは子が無い状態で開始され、大抵は自身の監視の _下に_ あり、後に任意の数のタスクの監視に使用することができます。
...
これでアプリケーションが指定されたどこのノードで開始されても、 `Chat.Supervisor` が開始されてタスクを監視する準備ができることがわかります。
...
コードに戻って何が起こっているのかをブレークダウンしてみましょう。
...
そのノードがチャットアプリケーションのインスタンス _も_ 実行していて、 `Chat.TaskSupervisor` がチャットアプリのスーパービジョンツリーの一部として開始されているため、 `Chat.TaskSupervisor` という名前でタスクスーパーバイザがそのノードで実行されていることがわかります。
...
`kate` ノードを開始すると、 `alex` も `moebi` も何も送っていないので、メッセージが来ていないことがわかります(ごめんね `kate`)。
