...
他の言語におけるオーバーロードとは違い、これらは互いに_異なる_関数として扱われます。
(さっき扱ったパターンマッチングは_同じ_数の引数を取る関数定義が複数ある場合のみ適用されます)
...
`:name` キーを_含まない_マップで関数を実行するとどうなるでしょうか？
...
`%{name: person_name} = person}` は `%{name: person_name}` が `person` に対してパターンマッチしているように見えたとしても、実際には_それぞれが_渡された引数をパターンマッチしているということを覚えておいてください。
