...
この目的はとてもシンプルです。
Erlangのノードは情報やリソースを共有（配布）するために他のノードに接続するために使われます。
...
2つ以上のノードが参加する分散システムを実行する場合、 `Mnesia.start/1` 関数をすべてのノードで実行する必要があることに留意するとよいでしょう。
...
存在しないレコードを照会しようとすると、Mnesiaは空のリストを返します。
...
以下のコードでは、以下のロジックを実装することでこれを実現しています。
