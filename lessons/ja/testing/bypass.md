...
私たちがGenServerを_作っていない_ことにお気づきでしょうが、これには十分な理由があります。
...
今回のプロジェクトでは、スケジューラはシンプルなものにし、 `Process.send_after/3` を利用して、繰り返しチェックを行うことにします。`Process` モジュールの詳細については、 [ドキュメント](https://hexdocs.pm/elixir/Process.html) を参照してください。
