...
우리는 "모의 객체 사용(mocking)"에 대해 주의 깊게 다루고 싶습니다. 우리는 주어진 테스트 예제에서 고유한 함수 스텁을 만들어서 특정 상호 작용할 때 위험한 패턴을 만듭니다. 테스트 실행을 API 클라이언트와 같은 특정 의존성 비헤이비어와 연결합니다. 스텁 함수 간에 공유하는 비헤이비어를 정의하려 하지 않습니다. 그것은 우리의 테스트를 반복하는 것을 더 어렵게 만듭니다.
