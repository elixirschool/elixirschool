...
현재, 엘릭서 커뮤니티는 에러를 반환하는 것에 대해 몇 가지 관례를 정했습니다.  

* 함수의 일반적인 작업에 포함되는 에러인 경우(예를 들어, 사용자가 날짜를 잘못 입력한 경우), 함수는 `{:ok, result}`와 `{:error, reason}`을 반환합니다.
* 일반적인 작업에 포함되지 않는 에러인 경우(예를 들어, 설정 데이터의 구문을 분석할 수 없는 경우), 함수는 예외를 발생시킵니다.  

우리는 일반적으로 [패턴 매칭](/ko/lessons/basics/pattern_matching)을 사용하여 보통의 에러 흐름을 다루지만, 이번 강의에서는 두 번째 사례인 예외에 초점을 맞추겠습니다.  

종종 Public API들에서 !가 포함된 두 번째 버전의 함수(example!/1)를 찾을 수 있으며, 이는 감싸지지 않은 결과를 반환하거나 에러를 발생시킵니다.
...
`try/catch`를 이용하여 종료를 잡아내는 것이 가능하지만, 그런 경우는 _극히_ 드뭅니다. 대부분의 경우, 슈퍼바이저가 프로세스 종료를 다루도록 하는 것이 유리합니다.
