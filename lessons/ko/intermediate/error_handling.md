...
## 일반적인 관례
현재, 엘릭서 커뮤니티는 에러를 반환하는 것에 대해 몇가지 관례를 정했습니다.  
...
우리는 일반적으로 [패턴 매칭](/ko/lessons/basics/pattern_matching)을 사용하여 보통의 에러 흐름을 다루지만, 이번 강의에서는 두번째 사례인 예외에 초점을 맞추겠습니다.  

종종 Public API들에서 !가 포함된 두번째 버전의 함수(example!/1)를 찾을 수 있으며, 이는 감싸지지 않은 결과를 반환하거나 에러를 발생시킵니다. 
