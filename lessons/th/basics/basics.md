...
ใน Elixir แนวคิดของ "ความเป็นจริง (truthiness)" นั้นเรียบง่ายมาก: เฉพาะ `false` และ `nil` เท่านั้นที่ถือว่าเป็นเท็จ ค่าอื่นๆ ทั้งหมด รวมถึง `0`, `""` (สตริงว่าง) และ `[]` (ลิสต์ว่าง) ถือว่าเป็นจริง กฎที่เข้มงวดนี้ทำให้ตัวดำเนินการบูลีนเช่น `||`, `&&` และ `!` ทำงานได้อย่างคาดเดาได้กับข้อมูลทุกประเภทสำหรับตรรกะเงื่อนไข
...
หมายเหตุ: `and` และ `or` ใน Elixir จริงๆ แล้วแมปไปยัง `andalso` และ `orelse` ใน Erlang
...
ในบทเรียนนี้ เราได้เรียนรู้องค์ประกอบพื้นฐานของ Elixir

เราเริ่มต้นด้วยการติดตั้ง Elixir และเปิด shell แบบโต้ตอบ IEx ซึ่งเราประเมินนิพจน์ง่ายๆ และเห็นผลลัพธ์ทันที จากนั้นเราสำรวจประเภทข้อมูลหลัก: จำนวนเต็ม (รวมถึงรูปแบบเลขฐานสอง ฐานแปด และฐานสิบหก) จำนวนทศนิยม บูลีน อะตอม และสตริง

เรายังทำงานกับการดำเนินการพื้นฐาน เช่น เลขคณิต ตรรกะบูลีน และตัวดำเนินการเปรียบเทียบ ระหว่างทาง เราเห็นว่า Elixir จัดการกับความเป็นจริงอย่างไร — เฉพาะ `false` และ `nil` เท่านั้นที่เป็นเท็จ — และตัวดำเนินการ `||`, `&&`, `and` และ `or` ทำงานแตกต่างกันอย่างไรขึ้นอยู่กับความคาดหวัง

สุดท้าย เราดูการแทรกค่าในสตริงและการต่อสตริง ซึ่งเป็นเครื่องมือสำคัญสองอย่างสำหรับการทำงานกับข้อความ

แนวคิดเหล่านี้เป็นรากฐานของการพัฒนา Elixir ในชีวิตประจำวัน ใช้เวลาทดลองใน IEx ลองแก้ไขตัวอย่าง และสังเกตว่าภาษาทำงานอย่างไร ความเข้าใจที่มั่นคงในพื้นฐานเหล่านี้จะทำให้หัวข้อถัดไปเข้าใจได้ง่ายขึ้นมาก
