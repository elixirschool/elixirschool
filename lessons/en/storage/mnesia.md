...
Unlike traditional databases that require separate installation and configuration, Mnesia runs inside our Elixir application's virtual machine, providing:

- **Distributed storage** across multiple nodes
- **Real-time performance** with in-memory and disk storage options
- **Fault tolerance** with automatic replication and recovery
...
- Do we need a database that's embedded within our application?
- Are we building a distributed system that needs to share data across multiple nodes?
- Do we need real-time performance with sub-millisecond response times?
- Does our application require high availability with automatic failover?
- Do we need to store and query Elixir data structures directly?

Mnesia might not be the best choice for:
- Applications requiring SQL compatibility
- Systems that need to share data with non-Erlang/Elixir applications
- Applications with massive datasets that don't fit in memory
- Projects that require extensive reporting and analytics
...
This command creates a new schema on the current node. After running this, we'll notice a new directory in our current working directory named something like `Mnesia.nonode@nohost` - this is where Mnesia stores its data files.

### Understanding Nodes

The `node()` function returns the current node name. In a standalone IEx session, this might be something like `:nonode@nohost`. In distributed systems, we'll work with named nodes:
...
Node names are important in Mnesia because they determine how data is distributed across our cluster.
...
Mnesia starts asynchronously, so if we need to ensure tables are ready before proceeding, use `wait_for_tables/2`:
...
**Table types:**
- `:set` - Each key appears once (default)
- `:ordered_set` - Ordered keys, useful for range queries
- `:bag` - Multiple values per key, but no duplicates
- `:duplicate_bag` - Multiple values per key, duplicates allowed

**Storage types:**
- `:ram_copies` - Memory only (fastest, lost on restart)
- `:disc_copies` - Memory + disk (good balance)
- `:disc_only_copies` - Disk only (slowest, minimal memory usage)
...
If no record is found, we'll get an empty list:
...
For performance-critical operations where we don't need transaction guarantees, Mnesia provides "dirty" operations:
...
### 2. Design Tables for Our Query Patterns

Create indices for fields we'll query frequently:
...
- Use `:ram_copies` for cache-like data
- Use `:disc_copies` for important data that needs to survive restarts
- Use `:disc_only_copies` for large datasets that don't fit in memory

### 4. Handle Network Partitions

In distributed systems, be prepared for network partitions:
...
- **`:set`** - Best for key-value lookups
- **`:ordered_set`** - Best when we need sorted data or range queries
- **`:bag`** - Best for one-to-many relationships
- **`:duplicate_bag`** - Best when duplicates are meaningful
...
- **Start simple**: Begin with basic tables and operations, then add complexity as needed
- **Think distributed**: Design our schema with distribution in mind from the beginning
- **Use transactions**: Wrap critical operations in transactions to ensure consistency
- **Monitor performance**: Keep an eye on table sizes and query patterns
- **Plan for growth**: Design our schema evolution strategy early
