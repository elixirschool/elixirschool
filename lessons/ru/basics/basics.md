...
В Elixir концепция «истинности» очень проста: только `false` и `nil` считаются ложными. Любое другое значение, включая `0`, `""` (пустая строка) и `[]` (пустой список), считается истинным. Это строгое правило позволяет булевым операторам, таким как `||`, `&&` и `!`, предсказуемо работать с любым типом данных для условной логики.
...
В этом уроке мы рассмотрели основные строительные блоки Elixir.

Мы начали с установки Elixir и запуска интерактивной оболочки IEx, где вычисляли простые выражения и видели результаты мгновенно. Оттуда мы исследовали основные типы данных: целые числа (включая двоичные, восьмеричные и шестнадцатеричные формы), числа с плавающей точкой, булевы значения, атомы и строки.

Мы также работали с базовыми операциями, такими как арифметика, булева логика и операторы сравнения. По пути мы увидели, как Elixir обрабатывает истинность — только `false` и `nil` являются ложными — и как операторы `||`, `&&`, `and` и `or` ведут себя по-разному в зависимости от своих ожиданий.

Наконец, мы рассмотрели интерполяцию и конкатенацию строк — два важных инструмента для работы с текстом.

Эти концепции составляют основу повседневной разработки на Elixir. Уделите время экспериментам с ними в IEx, попробуйте изменить примеры и понаблюдайте за поведением языка. Твёрдое понимание этих основ значительно облегчит освоение следующих тем.
