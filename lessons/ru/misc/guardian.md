%{
  ...
  excerpt: """
  [Guardian](https://github.com/ueberauth/guardian) — часто используемая библиотека, основанная на [JWT](https://jwt.io/) (JSON Web Token).
  """
}
---
...
Это всё — простые примеры полей в JWT.
Технология позволяет добавить дополнительную информацию, которой требует логика приложения.
Главное — не забывайте что этот токен должен быть коротким, ведь он должен поместиться в HTTP заголовке.
...
3. Проверить, что найден пользователь. Иначе — запретить доступ: plug `EnsureAuthenticated`.
...
Первый вариант сначала пытается найти токен в сессии, потом — в заголовках запроса.
Если находит, загружает ресурс.

Второй — требует наличия корректного проверенного токена типа "access".
