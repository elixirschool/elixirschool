%{
  ...
  У цьому уроці ми розглянемо, як отримати доступ до функціональності у стандартній бібліотеці разом зі сторонніми пакетами Erlang.
  """
}
---
...
Розширена стандартна бібліотека Erlang доступна з будь-якого коду Elixir у нашій програмі.
Модулі Erlang представлені атомами в нижньому регістрі, такими як `:os` та `:timer`.

Давайте використаємо `:timer.tc` для вимірювання часу виконання заданої функції:
...
Повний список доступних модулів можна знайти в [Erlang Reference Manual](http://erlang.org/doc/apps/stdlib/).
