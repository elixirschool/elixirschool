...
В Elixir концепція «правдивості» дуже проста: тільки `false` та `nil` вважаються хибними. Будь-яке інше значення, включаючи `0`, `""` (порожній рядок) та `[]` (порожній список), вважається правдивим. Це суворе правило дозволяє булевим операторам, таким як `||`, `&&` та `!`, працювати передбачувано з будь-яким типом даних для умовної логіки.
...
У цьому уроці ми розглянули основні будівельні блоки Elixir.

Ми почали з встановлення Elixir та запуску інтерактивної оболонки IEx, де оцінювали прості вирази та бачили результати миттєво. Звідти ми дослідили основні типи даних: цілі числа (включаючи двійкові, вісімкові та шістнадцяткові форми), числа з рухомою комою, булеві значення, атоми та рядки.

Ми також працювали з базовими операціями, такими як арифметика, булева логіка та оператори порівняння. По дорозі ми побачили, як Elixir обробляє правдивість — тільки `false` та `nil` є хибними — і як оператори `||`, `&&`, `and` та `or` поводяться по-різному залежно від своїх очікувань.

Нарешті, ми розглянули інтерполяцію та конкатенацію рядків — два важливі інструменти для роботи з текстом.

Ці концепції складають основу повсякденної розробки на Elixir. Приділіть час експериментам з ними в IEx, спробуйте змінити приклади та спостерігайте за поведінкою мови. Міцне розуміння цих основ значно полегшить засвоєння наступних тем.
