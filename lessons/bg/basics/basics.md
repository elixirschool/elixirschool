...
В Elixir концепцията за "истинност" е много проста: само `false` и `nil` се считат за неистинни. Всяка друга стойност, включително `0`, `""` (празен низ) и `[]` (празен списък), се счита за истинна. Това стриктно правило позволява на булевите оператори като `||`, `&&` и `!` да работят предвидимо с всякакъв тип данни за условна логика.
...
Забележка: `and` и `or` в Elixir всъщност съответстват на `andalso` и `orelse` в Erlang.
...
В този урок разгледахме основните градивни елементи на Elixir.

Започнахме с инсталирането на Elixir и стартирането на интерактивната обвивка IEx, където оценихме прости изрази и видяхме резултатите веднага. Оттам разгледахме основните типове данни: цели числа (включително двоични, осмични и шестнадесетични форми), числа с плаваща запетая, булеви стойности, атоми и низове.

Работихме също с основни операции като аритметика, булева логика и оператори за сравнение. По пътя видяхме как Elixir работи с истинност — само `false` и `nil` са неистинни — и как операторите `||`, `&&`, `and` и `or` се държат различно в зависимост от техните очаквания.

Накрая разгледахме интерполацията и конкатенацията на низове, два основни инструмента за работа с текст.

Тези концепции са основата на ежедневната разработка с Elixir. Отделете малко време, за да ги изпробвате в IEx, опитайте да промените примерите и наблюдавайте как езикът се държи. Солидното разбиране на тези основи ще направи следващите теми много по-лесни за усвояване.
