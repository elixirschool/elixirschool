...
في إليكسير، مفهوم "الصدق" بسيط للغاية: فقط `false` و `nil` يعتبران خاطئين. كل قيمة أخرى، بما في ذلك `0`، `""` (السلسلة الفارغة)، و `[]` (القائمة الفارغة)، تعتبر صحيحة. هذه القاعدة الصارمة تسمح لعوامل المنطق مثل `||`، `&&`، و `!` بالعمل بشكل متوقع مع أي نوع بيانات للمنطق الشرطي.
...
ملاحظة: عوامل `and` و `or` في إليكسير تُعيَّن فعلياً إلى `andalso` و `orelse` في إيرلانغ.
...
في هذا الدرس، تناولنا اللبنات الأساسية لإليكسير.

بدأنا بتثبيت إليكسير وتشغيل الصدفة التفاعلية IEx، حيث قمنا بتقييم تعبيرات بسيطة ورأينا النتائج فوراً. ومن هناك، استكشفنا أنواع البيانات الأساسية: الأعداد الصحيحة (بما في ذلك الصيغ الثنائية والثمانية والست عشرية)، والأعداد العشرية، والقيم المنطقية، والذرات، والسلاسل النصية.

كما عملنا مع العمليات الأساسية مثل الحساب والمنطق المنطقي وعوامل المقارنة. وعلى طول الطريق، رأينا كيف يتعامل إليكسير مع الصدق — فقط `false` و `nil` خاطئان — وكيف تتصرف العوامل مثل `||`، `&&`، `and`، و `or` بشكل مختلف حسب توقعاتها.

أخيراً، نظرنا إلى استيفاء السلاسل النصية وربطها، وهما أداتان أساسيتان للعمل مع النصوص.

هذه المفاهيم تشكل أساس التطوير اليومي بإليكسير. خذ بعض الوقت لتجربتها في IEx، وحاول تعديل الأمثلة، ولاحظ كيف تتصرف اللغة. الفهم الراسخ لهذه الأساسيات سيجعل المواضيع القادمة أسهل بكثير في الاستيعاب.
