...
この例では、リクエストが幾つかの必要な引数を持っているかどうかを確かめるPlugを作ります。Plugにバリデーションを実装することで、有効なリクエストのみがアプリケーションに渡されることを確信することができます。ここで作るPlugは2つのオプション、`:paths`と`:fields`で初期化されると想定します。これらはロジックを適用するパスと、必要なフィールドを表しています。
