...
Теперь мы можем создавать наборы изменений, но поскольку у нас ещё нет никакой валидации, любое изменение имени будет принято. Например, полностью пустое имя:
...
Ecto утверждает, что набор изменений прошёл валидацию, но нам бы не хотелось допускать пустых имён. Попробуем исправить!
...
Обычно для каждой схемы для создания набора изменений объявляют одну или несколько специальный функций. Мы поступим так же. Наша функция будет принимать структуру и словарь с изменениями, а возвращать набор изменений:
...
Можно сделать так, чтобы автоматическое имя пользователи получали только при регистрации. Для этого мы создадим новую функцию-конструктор набора изменений:
...
Иметь разные функции для создания разных наборов изменений (как `registration_changeset/2` выше) — это распространённая практика. В разных ситуациях требуется разная валидация полей и параметров.
