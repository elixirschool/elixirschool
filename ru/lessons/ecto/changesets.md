...
При создании, изменении или удалении записей `Ecto.Repo.insert/2`, `update/2` и `delete/2` принимают набор изменений (чейнджсет) в качестве первого параметра. Но что такое эти чейнджсеты?
...
## Создание первого чейнджсета

Давайте взглянем на пустую `%Changeset{}` структуру:
...
Чтобы начать использовать чейнджсет, необходимо при создании как-то сообщить ему информацию о том, как будут выглядеть данные.
...
Чтобы создать чейнджсет при помощи схемы `Person`, воспользуемся функцией `Ecto.Changeset.cast/4`:
...
Вторым по череди идёт изменение, которое мы ходим осуществить — пустой словарь.
Третий параметр специфичен для `cast/4` — это список полей, которым мы разрешаем измениться. Таким образом у нас появляется полный контроль над тем, какие поля могут быть изменены в ходе операции, а какие должны остаться нетронутыми.
...
Это полезно в случаях, когда мы доверяем источнику изменений или уже проверили данные вручную.

Теперь мы можем создавать чейнджсеты, но поскольку у нас ещё нет никакой валидации, любое изменение имени будет принято. Например, полностью пустое имя:
...
Ecto говорит, что чейнджсет прошёл валидацию, но нам бы не хотелось допускать пустых имён. Попробуем исправить!

## Валидация

В Ecto включён ряд встроенных функций для валидации.

Мы будем активно использовать модуль `Ecto.Changeset`, поэтому давайте импортируем его в наш модуль `person.ex`. Туда, где уже находится наша схема:
...
Обычно для каждой схемы для создания чейнджсета объявляют одну или несколько специальный функций. Давайте создадим одну. Она будет принимать структуру и словарь с изменениями, а возвращать чейнджсет:
...
Если мы вызовём функцию `Friends.Person.changeset/2` и передадим в неё пустое имя, то чейнджсет не пройдёт валидацию и даже услужливо скажет нам об этом.
...
При попытке выполнить `Repo.insert(changeset)` с чейнджсетом из примера выше мы получим `{:error, changeset}` с той же самой ошибкой в качестве результата. Поэтому не обязательно выполнять `changeset.valid?` каждый раз.
...
Хотя встроенные валидаторы и закрывают довольно широкий спектр потребностей, всё еще может возникнуть необходимость чего-то особенного.

Каждая `validate_`-функция из тех что мы уже использовали принимала и возвращала чейнджсет `%Ecto.Changeset{}`, поэтому нам не составит труда написать свою.
...
Отлично, работает! Впрочем, не было никакой нужды писать эту функцию самостоятельно — мы могли использовать валидатор `validate_inclusion/4`. Зато мы научились добавлять нужные нам ошибки.

## Программное изменение значений

Иногда нам нужно вручную сделать изменения в чейнджсете. Для этого можно использовать функцию `put_change/3`.
...
Нужная нам функция выглядит знакомо — она принимает и возвращает чейнджсет, точно так же как и `validate_fictional_name/1`:
...
Можно сделать так, чтобы автоматическое имя пользователи получали только при регистрации. Для этого мы создадим новую функцию-конструктор чейнджсета:
...
Иметь разные функции для создания разных чейнджсетов (как `registration_changeset/2` выше) — это распространённая практика. В разных ситуациях требуются разная валидация полей и параметров.
...
Существует ещё много возможностей и механизмов, не затронутых в этом уроке. Таких как, например, [бессхемные чейнджсеты](https://hexdocs.pm/ecto/Ecto.Changeset.html#module-schemaless-changesets), которые можно использовать для валидации _любых_ данных. Или вызов сайд-эффектов в ходе обработки чейнджсета ([`prepare_changes/2`](https://hexdocs.pm/ecto/Ecto.Changeset.html#prepare_changes/2)). Или работа с ассоциациями и вложенными структурами.
