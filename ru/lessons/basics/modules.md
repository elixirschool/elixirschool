...
Это всё, что `use` делает. Однако, часто для макроса `__using__` вызывается `alias`, `require`, или `import`. Это, в свою очередь, создает псевдонимы или импорты в используемом модуле. Данная возможность позволяет использовать модуль для определения политики о том, как следует ссылаться на его функции и макросы. Макрос `__using__/1` предоставляет возможность устанавливать ссылки на другие модули и подмодули, что делает его довольно гибким.
