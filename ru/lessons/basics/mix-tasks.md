...
Примечание: Наш код должен быть скомпилирован до того, как новые задачи появятся в выводе `mix help`.
Мы можем сделать это либо запустив `mix compile', либо запустив нашу задачу, как мы это делали с `mix hello`, которая запустит компиляцию.