...
Используя его в сочетании с другими особенностями Elixir, такими как документация, которая является объектом первого класса, как мы только что видели, можно невероятно расширить возможности разработчика.
...
Если вам нужно разбить коллекцию на меньшие группы, метод `chunk/2` - это то, что нужно:
...
Если нам нужно разделить коллекцию по какому-то другому признаку кроме количества, мы можем воспользоваться функцией `chunk_by/2`. Она принимает перечисление и функцию. Когда возвращаемое функцией значение изменяется, начинается формирование новой группы:
...
```elixir
# Применяем функцию на каждый 3-й элемент
...
Функция `max/1` возвращает максимальное значение в коллекции:
...
Сортировка коллекций реализуется не одной, а двумя функциями `sort`.
...
Мы можем использовать `uniq/1` для удаления дубликатов из перечислений:
