...
* **99th %** - 99% процентов всех замеров быстрее этого, что в какой-то степени делает его показателем худшей производительности. Чем меньше, тем лучше.
...
* **save** - определяет путь (`path`) для хранения результатов текущего набора замеров, с пометкой специальным тегом (`tag`). См. [Сохранение & Загрузка в документации Benchee](//github.com/bencheeorg/benchee#saving-loading-and-comparing-previous-runs).
* **load** - загружает сохранённый набор(ы) замеров для сравнения с текущими. Может быть строкой или списком строк или паттернов. См. [Сохранение & Загрузка в документации Benchee](//github.com/bencheeorg/benchee#saving-loading-and-comparing-previous-runs).
...
Все они делают именно то, что можно было бы ожидать исходя из их названия: они пишут результат в файлы соответствующего формата, чтобы затем с ними можно было работать любым удобным способом.
...
Мы почти дошли до конца, но сделали это, не показав вам одну из крутейших функций Benchee: замеры памяти!
