...
`unless/2` s'utilise come `if/2` mis à part que ca fonctionne sur la négation:
...
Comme `case`, `cond` génèrera une erreur s'il n'y a pas de correspondance. Pour palier à ca, on peux définir une condition `true`:
