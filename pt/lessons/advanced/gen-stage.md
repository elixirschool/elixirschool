...
Nesta lição vamos examinar mais de perto o GenStage, para o que serve e como podemos usá-lo em nossas aplicações.

{% include toc.html %}

## Introdução

Então, o que é GenStage ? Na documentação oficial, é uma "especificação e fluxo computacional para Elixir", mas o que isso significa para nós ?

Significa que o GenStage fornece uma maneira de definir um pipeline de trabalho a ser realizado por passos independentes (ou etapas) em processos separados; Se você já trabalhou com pipelines antes, então alguns desses conceitos devem ser familiares.

Para entender melhor como isso funciona, vamos visualizar um fluxo produtor-consumidor simples:
...
Neste exemplo temos três etapas: `A` um produtor, `B` um produtor-consumidor, e `C` um consumidor.  `A` produz um valor que é consumido por `B`, `B` executa alguns trabalhos e retorna um novo valor que é recebido pelo nosso consumidor `C`; a regra para nossa etapa é importante como vamos ver na próxima seção.

Enquanto nosso exemplo é 1-para-1 produtor-consumidor é possível que ambos tenham múltiplos produtores e múltiplos consumidores em qualquer fase.

Para melhor ilustrar esses conceitos, iremos construir um pipeline com o GenStage, mas primeiro vamos explorar as funções que o GenStage depende um pouco mais.


## Consumidores e Produtores

Como lemos, o papel que damos a nossa etapa é importante. A especificação GenStage reconhece três funções:

+ `:producer` — Uma fonte. Produtores aguardam por demanda de consumidores e responde com eventos solicitados.

+ `:producer_consumer` — Tanto uma fonte como uma pia. Produtores-consumidores podem responder à demanda de outros consumidores, bem como solicitar eventos aos produtores.
